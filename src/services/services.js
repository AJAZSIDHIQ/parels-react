...
class Services {
  ...
  createAccount = (phoneNumber) =>
    this.auth.createUserWithEmailAndPassword(phoneNumber);

  signIn = async (phoneNumber) =>  {
    // this.auth.signInWithEmailAndPassword(phoneNumber);

    const res = await api.post("/api/v1/auth/login/mobile" ,{
      "countryCode": "+91",
      "mobileNumber": phoneNumber
    });
  }

  validateMobile = async (phoneNumber , otp_value) => {
    const res = await api.post("/api/v1/auth/login/mobile/validate" ,{
      "countryCode": "+91",
      "mobileNumber": phoneNumber,
      "otp": otp_value
    });
