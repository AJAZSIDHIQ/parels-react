import {
  ...
  SIGNOUT, SIGNUP , VALIDATE_MOBILE
...
function* authSaga({ type, payload }) {
  switch (type) {
    case SIGNIN:
      try {
        yield call(services.signIn, payload.phoneNumber);
      } catch (e) {
        yield handleError(e);
      }
      break;
    ...
    case SIGNIN_WITH_GOOGLE:
      try {
        yield initRequest();
        yield call(services.signInWithGoogle);
      } catch (e) {
        yield handleError(e);
      }
      break;
    case SIGNIN_WITH_FACEBOOK:
      try {
        yield initRequest();
        yield call(services.signInWithFacebook);
      } catch (e) {
        yield handleError(e);
      }
      break;
    case SIGNIN_WITH_GITHUB:
      try {
        yield initRequest();
        yield call(services.signInWithGithub);
      } catch (e) {
        yield handleError(e);
      }
      break;
    case SIGNUP:
      try {
        ...
        const ref = yield call(services.createAccount, payload.email, payload.password);
        ...
        yield call(services.addUser, ref.user.uid, user);
    ...
    case RESET_PASSWORD: {
      try {
        yield initRequest();
        yield call(services.passwordReset, payload);
        yield put(setAuthStatus({
          success: true,
          type: 'reset',
    ...
    case ON_AUTHSTATE_SUCCESS: {
      const snapshot = yield call(services.getUser, payload.uid);
      ...
      if (snapshot.data()) { // if user exists in database
        ...
        yield call(services.addUser, payload.uid, user);
        yield put(setProfile(user));
        yield put(signInSuccess({
          id: payload.uid,
          role: user.role,
    ...
    case SET_AUTH_PERSISTENCE: {
      try {
        yield call(services.setAuthPersistence);
      } catch (e) {
        console.log(e);
      }
      break;
